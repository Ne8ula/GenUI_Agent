...
const result={kind:'native Rust/wgpu visual capture driven by CDP; OS input injection blocked; no mic/provider calls',checks:[],errors:[]};
...
try{
 ...
 probe(['-Action','Capture','-BringFixtureToFront','-RaiseE1ProcessId',nativePid]);await main.waitForTimeout(500);capture('idle');
 await click('Controls +');await main.getByRole('button',{name:'Mute answer',exact:true}).waitFor({state:'visible',timeout:4000});
